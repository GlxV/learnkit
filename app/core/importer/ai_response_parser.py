...
class AIResponseParser:
    SECTION_NAMES = ("RESUMO", "FLASHCARDS", "PERGUNTAS")
    ...
    def parse(self, raw_text: str) -> ParsedAIResponse:
        ...
        summary = Summary(content=sections.get("RESUMO", "").strip())
        ...
        return ParsedAIResponse(ai_response, summary, flashcards, questions, warnings)

    def _split_sections(self, raw_text: str) -> dict[str, str]:
        matches = list(re.finditer(r"(?im)^#\s*(RESUMO|FLASHCARDS|PERGUNTAS)\s*$", raw_text))
